...
import { color, Box, Flex } from "../../components/Style"
import Layout, { globalGap, apiURL, RowMaxWidth } from "../../components/Layout"
...
export default () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const result = await axios(apiURL + "/ezessayblogs/" + parameter["id"])
    ...
    <Layout>
      ...
      <RowMaxWidth>
        <Box mb={globalGap}>
          ...
          <Flex py={1.5} bb={color.divider} gap={globalGap}>
            <Box o={0.5}>编辑：{data && data.author}</Box>
            <Box o={0.5}>时间：{data && dateFormatter(data.time)}</Box>
          </Flex>
        </Box>
